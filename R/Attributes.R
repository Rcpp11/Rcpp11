...
sourceCpp <- function(file = "",
    ...
    context <- .Call("sourceCppContext", PACKAGE="Rcpp11", file, code, rebuild, .Platform)
...
compileAttributes <- function(pkgdir = ".", verbose = getOption("verbose")) {
    ...
    # generate exports
    invisible(.Call("compileAttributes", PACKAGE="Rcpp11",
...
.findPlugin <- function(pluginName) {
    ...
    if (is.null(plugin))
        stop("Inline plugin '", pluginName, "' could not be found ",
             "within the Rcpp11 package. You should be ",
...
.setupBuildEnvironment <- function(depends, plugins, sourceFile) {
    ...
    # if there is no buildEnv from a plugin then use the Rcpp11 plugin
...
.pathWithRtools <- function() {
    ...
    if (!hasRtools) {
        ...
        if (!is.null(key)) {
            ...
            if (identical("2.15", ver) || identical("2.16", ver) || identical("3.0", ver) || identical("3.1", ver) ) {

                # See if the InstallPath leads to the expected directories
                rToolsPath <- key$`InstallPath`
                if (!is.null(rToolsPath)) {

                    # Return modified PATH if expected directories exist
...
.linkingToIncludes <- function(linkingTo, pluginsOnly) {
    ...
    # Look for Rcpp11 inline plugins within the list or LinkedTo packages
    include.before <- character()
    include.after <- character()
    linkingToPackages <- .parseLinkingTo(linkingTo)
    for (package in linkingToPackages) {

        # We already handle Rcpp11 internally
        if (identical(package, "Rcpp11"))
            next
