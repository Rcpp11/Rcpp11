...
namespace Rcpp{
    ...
    public:
        ...
        Vector( SEXP x, bool coerce = false) {
            if (!coerce && TYPEOF(x) != RTYPE) {
                stop("Attempted to construct a '%s' from a vector of different type '%s'",
                    CHAR(Rf_type2str(RTYPE)), CHAR(Rf_type2str(TYPEOF(x))));
            }
